...
@Component({
  selector: 'app-dynamic-form-array-preview-v2',
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule],
  templateUrl: './dynamic-form-array-preview-v2.component.html',
  styleUrl: './dynamic-form-array-preview-v2.component.scss',
})
export class DynamicFormArrayPreviewV2Component implements OnChanges {
  @Input() formPreviewArray!: FormArray;
  @Input() formPreviewConfig: any[] = [];
  @Input() itemPreviewConfig: any[] = [];
  @Input() dynamicForm!: FormGroup;
  @Input() formValue: any;
  @Input() formPreviewArrayName!: string;
  @Input() labelFormPreviewArray!: string;
  ...
  onSubmit(action: string): void {
    const formData = { ...this.dynamicForm.value, action: action };
  ...
  filterOptions(event: Event, key: string): void {
    ...
    const field = this.formPreviewConfig.find((f) => f.key === key);
